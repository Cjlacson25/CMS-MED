...
interface PhysiciansandProvidersProps {}

const PhysiciansandProviders: React.FC<PhysiciansandProvidersProps> = () => {
	const [loading, setLoading] = useState(false);
	const [physiciansProviders, setPhysiciansProviders] = useState<any>(() => ({}));
	const [company, setCompany] = useState<any[]>(() => []);
	const [companySearch, setCompanySearch] = useState(1);
	const initialize = useCallback(() => {
		setLoading(true);
		api()
			.get<any>('physician-providers/', { params: { search: { CompanyID: companySearch } } })
			.then(({ data }) => {
				setPhysiciansProviders(data.response?.[0]);
			})
			.finally(() => {
				setLoading(false);
			});
	...
	const handleSubmit = async (values: any) => {
		...
		try {
			await api().post<any>('physician-providers/create-update', formData);
			initialize();
		} catch {}
	};
	return (
		<Page title="Physicians and Providers">
			<HForm
				dialog
				loading={loading}
				enableReinitialize
				initialValues={{
					Content: physiciansProviders?.Content ?? '',
					ImagePath: physiciansProviders?.ImagePath ?? '',
					CareerPhysicianProviderID: physiciansProviders?.CareerPhysicianProviderID,
					isActive: 1,
					isPublished: 1,
				}}
				...
				<Grid container spacing={1} className="flex flex-col flex-grow">
					<Grid container rowSpacing={2} columnSpacing={5}>
						<Grid item xs={12} md={6}>
							<div className="font-bold text-2xl mx-2 my-3">{'Physicians and Providers'}</div>
						</Grid>
...
export default PhysiciansandProviders;
